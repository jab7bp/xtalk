...
TH2D *h2_chan_1_2 = new TH2D("h2_chan_1_2", "", 128, 0, 127, 128, 0, 127);
...
int first_event = 0;
int last_event = 200000;
double ADC_max_thresh = 4000;
double apv_adc_channels[128][200000];
...
void xtalk_correlation(int const ADC_cut = 500){
	...
	//where that main loop starts.
		...
		//Define histograms:
		for( int i = 0; i < 128; i++){
			h_channels_ADCs[i] = new TH1D(Form("h_strip_%i_ADCs", i), "", 3000, 0, 3000);
		}
	...
	// EVENT //Loop over selected events:
			...
			cout << "**************************************************" << endl;
			for(int evt = first_event; evt <= last_event; evt++){
	...
	//------------Threshold ADC cut on all apvs (Can maybe implement this on the pedestal RMS CUT???)
			...
			for(int i = 0; i < 128; i++){
				for(int j = 0; j < last_event; j++){
					h_channels_ADCs[i]->Fill(apv_adc_channels[i][j]);
					if(j%10000 == 0){cout << "On event: " << j << endl;}
				}
			}
			//END of EVENTS
			cout << "**************************************************" << endl;
...
//CALCULATE THE CORRELATION COEFFICIENT
		...
		// }
		for(int i = 0; i < 128; i++){
			h_chan1_corr_coeff->Fill(correlation_coeff_2D[0][i]);
			h_chan2_corr_coeff->Fill(correlation_coeff_2D[1][i]);
			h_chan3_corr_coeff->Fill(correlation_coeff_2D[2][i]);
		}
		for(int i =0; i < 128; i++){
			h2_chan_1_2->Fill(h_chan1_corr_coeff->GetBinContent(i), h_chan2_corr_coeff->GetBinContent(i));
			for(int j = 0; j < 128; j++){
