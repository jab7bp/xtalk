...
void crosstalk() {
	...
	for(double_t i = 0; i < max_entries/2; i++) {
	...
	for(double_t i=0; i < 5000; i++) {
		float ADC1 = gRandom->Landau(1500, 300);
		float ADCrand = gRandom->Gaus(0.2, (0.2)/10);
		float ADC2 = ADC1*ADCrand;
		float r;
	...
	hlandau1->Draw();
	hlandau1->SetTitle("Landau Distribution For Signal on Channel 'n'");
	hlandau1->GetXaxis()->SetTitle("ADC");
	hlandau1->GetYaxis()->SetTitle("Entries");
	c1->Update();

	TCanvas * c2 = new TCanvas("c2", "landau2", 200, 10, 600, 600);
	hlandau2->Draw();
	hlandau2->SetTitle("Landau Distribution For Crosstalk Signal on Channel 'n+1'");
	hlandau2->GetXaxis()->SetTitle("ADC");
	hlandau2->GetYaxis()->SetTitle("Entries");
	c2->Update();
	
	TCanvas * cratio = new TCanvas("cratio", "ratio", 200, 10, 600, 600);
	ratio->Draw();
	ratio->SetTitle("Ratio of ADC Values on Neighboring Channels");
	ratio->GetXaxis()->SetTitle("ADC Ratio");
	ratio->GetYaxis()->SetTitle("Entries");
	cratio->Update();
